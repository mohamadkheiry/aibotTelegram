...
- متن معمولی escape می‌شود؛ HTML فقط از `html:` معتبر یا تبدیل کنترل‌شده entityهای UTF-16 تلگرام با `telegram_input_text` مجاز است. متن خام را پیش از تبدیل offsetها strip نکنید.
...
- اعلان تیکت: `app/ticket_ui.py` سازنده مشترک مسیر پاسخ فوری و outbox است. متن/markup legacy را بازنویسی نکنید؛ بازگشایی مشتری status، notice و `ticket_reply` را اتمیک می‌کند و replay نباید state مکالمه بعدی را reset کند. FAQ درختی و جست‌وجوی نخستِ پیام تکی از فرم/journal موجود استفاده می‌کنند؛ قرارداد `docs/PENDING_FEEDBACK_2026-09-09.md` و تست‌های همان حوزه را حفظ کنید.
- وضعیت دسترسی عمومی: صفحهٔ اول هر دو PDF فقط خاموش/روشن‌کردن را تعریف می‌کند. `AdminButtonUI.bot_status` باید فقط یک دکمهٔ معکوس وضعیت `bot_enabled` بدهد؛ دکمهٔ تکراری یا flag مستقل تعمیرات نسازید. متن فعلی/مقصد، تأیید، هدف صریح callback قدیمی و دسترسی مدیر حفظ شود. قرارداد UI-A07 در `docs/BUTTON_UI_AUDIT.md` و ۱۳ تست `test_admin_bot_status.py` است.
- آیکون‌های اختیاری Premium: قرارداد `docs/BUTTON_ICONS.md` و registry واحد `app/button_icons.py`. فقط کپی payload در مرز `LayoutEngine.prepare` تزئین می‌شود؛ آیکون صریح و label/action/outbox اصلی ثابت‌اند. کلیدهای manifest عمومی‌اند، نه secret یا file ID. طبق تأکید مالک رنگ‌بندی `colored` در انتشار جدید نیز حفظ می‌شود؛ افزودن آیکون یا inline شیشه‌ای مجوز تغییر به `theme` نیست. `theme` فقط fallback اختیاری با تأیید مالک است، نه CSS glass سفارشی. ابزار انتشار بسته بدون poller/DB است؛ تغییر bot ID نیازمند DB مستقل و تصمیم صریح دامنه انتقال داده است.
- چیدمان مشتری: پیش از تغییر هر keyboard کاربر، `docs/CUSTOMER_LAYOUTS.md` را بخوانید. registry واحد ۴۲ نوع صفحه در `app/customer_layouts.py`، ویرایشگر در `app/admin_layouts.py` و ذخیرهٔ CAS/undo/journal در `Database.save_customer_layout` است. چیدمان سند اکنون پیش‌فرض قابل بازنشانی است؛ payload/مجوز/شرط نمایش تغییر نمی‌کند. metadata داخلی پیش از Telegram حذف و outbox canonical برای ظاهر بازنویسی نمی‌شود. صفحهٔ جدید بدون section و تست پوشش مجاز نیست.
