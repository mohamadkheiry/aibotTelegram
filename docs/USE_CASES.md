...
**مسیر دکمهٔ ربات/تعمیرات:** در حالت فعال، «غیرفعال‌کردن ربات» یا «فعال‌کردن حالت تعمیرات» فرم هدف `bot_off` را باز می‌کند؛ در حالت غیرفعال، «فعال‌کردن ربات» یا «غیرفعال‌کردن حالت تعمیرات» هدف `bot_on` دارد. وضعیت فعلی و نتیجهٔ تأیید نمایش داده می‌شود؛ لغو اثر ندارد. پس از تأیید، هر دو دکمه عمل معکوس را نشان می‌دهند و مدیر همچنان دسترسی دارد. callback قدیمی هدف صریح قبلی را حفظ می‌کند، نه toggle ضمنی؛ تأیید مصرف‌شده نمی‌تواند تغییر بعدی مدیر را برگرداند. ۱۱ تست اختصاصی: `tests/test_admin_bot_status.py`.
