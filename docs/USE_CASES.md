...
تکمیل UX در [پیگیری ۲۰۲۶-۰۹-۰۹](PENDING_FEEDBACK_2026-09-09.md): مشتری پس از تأیید بازگشایی تیکت مستقیماً پیام می‌فرستد و اعلان پاسخ، دکمه پاسخ/بستن دارد؛ مدیر دسته FAQ و پرسش را به‌ترتیب باز می‌کند و همان مورد را ویرایش می‌کند. پیام تکی با جست‌وجوی مخاطب آغاز می‌شود. فرم تأیید و کنترل دسترسی همچنان الزامی‌اند.

## UC-UI-04 — تنظیم چیدمان کاربران
...
Actor مالک/مدیر اثبات‌شده در private chat، از مدیریت کلی ربات وارد جوین اجباری می‌شود. فهرست، هر کانال را با ✅/❌، سپس افزودن و بازگشت نشان می‌دهد. انتخاب کانال سه دکمه تغییر وضعیت، حذف و بازگشت دارد. افزودن/تغییر/حذف با فرم مشترک و تأیید نهایی انجام می‌شوند؛ لغو اثر ندارد، دوبار تأیید اثر دوم نمی‌سازد و بازگشت صفحه مربوط را حفظ می‌کند. کانال حذف‌شده به فهرست تازه، نقش لغوشده به رد دسترسی، و نبود adminبودن ربات به خطای بدون mutation می‌رسد. [پیش‌شرط‌ها، جریان‌های خطا و شواهد](ADMIN_JOINS.md).

## UC-UI-02 — مدیریت کالا از درخت دسته‌ها

Actor: مدیر یا مالک فعال و اثبات‌شده در private chat. جریان اصلی: ورود به یکی از ۹ بخش («محصولات»)، انتخاب دسته و زیردسته، انتخاب محصول، مشاهده مقدار کامل مشخصه و ویرایش آن، یا ورود به انبار/فرمت همان محصول. محصول و مشخصه انتخاب‌شده در فرم ثابت می‌مانند؛ فقط مقدار جدید و تأیید نهایی لازم است. پس از موفقیت یا لغو، دکمه بازگشت به زمینه انتخاب‌شده وجود دارد. ایجاد محصول/زیردسته نیز والد را از صفحه فعلی می‌گیرد.

جریان‌های جایگزین: صفحه‌بندی و جست‌وجو فقط در همان دسته/انبار؛ مخفی‌بودن مانع مدیریت نیست؛ محصول حذف‌شده یا موجودی متعلق به محصول دیگر رد می‌شود؛ پس از انتقال محصول، بازگشت دسته جدید را نشان می‌دهد؛ حذف دسته غیرخالی ممنوع و حذف محصول soft-delete است. تغییر ready به manual با موجودی، رزرو فعال یا سفارش آمادهٔ باز رد می‌شود؛ هیچ داده‌ای خودکار حذف نمی‌شود. role لغوشده، تأیید تکراری، crash و دکمه قدیمی نباید اثر تازه بسازند. پس‌شرط: همان رکورد انتخاب‌شده از handler مشترک تغییر کرده یا بدون تغییر رد شده است. [قرارداد و شواهد تست](ADMIN_HIERARCHY.md).

## UC-UI-01 — اداره ربات با دکمه

تکمیل جریان جایگزین: اقدامِ دکمهٔ قدیمی رد می‌شود، ولی فرم فعلی با دکمه‌های معتبر دوباره نمایش داده می‌شود؛ فرم پایان‌یافته برای فهرست‌ها «نمایش دوباره نتیجه» با حفظ فیلتر/صفحه دارد. پس از ادامه/پایان/لغو یا `/start`، markup مصرف‌شده در صورت امکان edit پاک می‌شود. این رفتار UI-A01/UI-A02 در [BUTTON_UI_AUDIT.md](BUTTON_UI_AUDIT.md) آزموده شده و نقش لغوشده را بازیابی نمی‌کند.

پیش‌شرط: private chat، شروع ربات و هویت اثبات‌شده مدیر برای پنل. actor مشتری منوی اصلی ثابت سند و actor مدیر ردیف اضافه پنل را می‌بیند. مدیر بخش/عملیات را انتخاب، رکورد را جست‌وجو و با دکمه برمی‌گزیند، داده آزاد را وارد، خلاصه را مرور و عملیات تغییردهنده را تأیید می‌کند. نتیجه با دکمه عملیات مرتبط و navigation ارائه می‌شود. تاریخ، enum و نوع عمل انتخابی‌اند؛ شرط ترکیبی نیاز به JSON ندارد.

جریان جایگزین: مرحله قبل پاسخ‌های وابسته را حذف می‌کند؛ لغو فرم هیچ mutation تازه‌ای ندارد؛ خطای ورودی اجازه اصلاح می‌دهد. revoke نقش و دکمه منقضی/فرم دیگر رد می‌شوند. خطای موقت DB همان update/اثر را بازیابی می‌کند و تأیید تکراری عملیات مالی دوم نمی‌سازد. خطای پاسخ Telegram ممکن است نیازمند مشاهده نتیجه از فهرست باشد. پیام گروهی preview دارای count و تأیید قبلی را حفظ می‌کند.

پس‌شرط: تغییر مجاز از همان handler و تراکنش دامنه انجام شده یا بدون تغییر رد/لغو شده است. مشتری از فرم‌های نام، تماس، شارژ، تخفیف، فیش، اطلاعات و تیکت با دکمه خارج می‌شود؛ این خروج refund یا لغو مالی نیست. جزئیات ۸۳ مسیر، سازگاری اعلان‌های قدیمی و تست‌ها در [BUTTON_UI.md](BUTTON_UI.md) است.
...
- اگر دسترسی در میانهٔ فرم تغییر کرد، «لغو و بازگشت»/`/cancel` state ورودی را پاک می‌کند ولی بدون عبور از خاموشی/جوین، منوی عمومی را نشان نمی‌دهد؛ تست `test_cancel_cannot_bypass_disabled_bot_or_forced_join`.
...
مرز رابط: `processing` برای ready نیز ممکن است وجود داشته باشد؛ دکمهٔ اطلاعات تنها با snapshot نوع manual مجاز است. callback تاریخی ready پیش از بازکردن فرم رد می‌شود و state اطلاعاتِ باقی‌مانده از نسخه قدیمی، بدون ثبت payload پاک می‌شود. سه تست ready در `tests/test_spec_end_to_end.py` این مرز را همراه سفر کامل محصول manual بررسی می‌کنند.
...
**محرک:** دکمه‌های «مدیریت کلی ربات»؛ فرمان‌های سازگار `/bot_on|off`, `/payment`, `/set_card`, `/set_channel`, `/joins`, `/join_*` اختیاری‌اند.
...
**مسیر تنها دکمهٔ وضعیت ربات:** مطابق صفحهٔ اول هر دو PDF، یک قابلیت خاموش/روشن وجود دارد؛ تعمیرات وضعیت مستقلی نیست. در حالت فعال، فقط «غیرفعال‌کردن ربات» با هدف `bot_off` و در حالت غیرفعال، فقط «فعال‌کردن ربات» با هدف `bot_on` نمایش داده می‌شود. وضعیت فعلی و اثر تأیید بر دسترسی کاربران روشن است؛ لغو اثر ندارد. پس از تأیید/بازیابی یک دکمهٔ معکوس می‌آید و مدیر همچنان دسترسی دارد. callback قدیمی، از جمله عنوان تاریخی تعمیرات، هدف صریح قبلی را حفظ می‌کند؛ تأیید مصرف‌شده نمی‌تواند تغییر بعدی مدیر را برگرداند. ۱۳ تست اختصاصی: `tests/test_admin_bot_status.py`؛ ممیزی UI-A07 در `BUTTON_UI_AUDIT.md`.

**مسیر انتخاب روش پرداخت:** پس از انتخاب wallet/card/crypto، نام فارسی همان روش و وضعیت فعلی در مرحله انتخاب وضعیت جدید و تأیید دیده می‌شوند؛ هدف تازه هنوز اعمال نشده است. نتیجه و بازیابی نیز وضعیت زنده دارند. بازگشت/تغییر روش، زمینه قبلی را پاک می‌کند؛ کارت یا سرویس ارزی ناقص هشدار بدون افشای مقادیر پیکربندی دارد. بررسی نقش، تأیید و منع فعال‌سازی ناقص تغییر نمی‌کند. ۱۲ تست: `tests/test_admin_payment_context.py`.
...
در رابط دکمه‌ای، inactive با عنوان «بدون فعالیت در چند روز اخیر» نشان داده می‌شود؛ مبنا آخرین فعالیت است نه نبود خرید. شاهد: `test_inactive_filter_is_labelled_by_activity_not_purchases`؛ اصلاح کیف پول از همین رابط نیز در سفر خرید ترکیبی تا گزارش مالی آزموده می‌شود.
...
در فرم دکمه‌ای combined، دعوت‌های ثبت‌شده و واجد پاداش متعلق به خودِ خریدارِ معرفی‌شده‌اند؛ عنوان مرحله و خلاصه نباید این فرد را با دعوت‌کنندهٔ دریافت‌کننده پاداش اشتباه بگیرند. شاهد رابط: `test_combined_reward_editor_names_the_buyer_whose_referrals_are_evaluated`؛ ارزیابی دامنهٔ قواعد همچنان در تست‌های پاداش پوشش دارد.
