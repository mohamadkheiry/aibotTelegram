...
- فیش فقط برای payment کارت است: نخستین فیش strictly پیش از `expires_at` و replacement فقط strictly پیش از `expires_at + 7 days` پذیرفته می‌شود؛ grace با جایگزینی تمدید نمی‌شود. payment دارای فیش/`verifying` با دکمه قدیمی کاربر لغو نمی‌شود و در صورت بلاتکلیفی پس از grace منقضی می‌گردد.
