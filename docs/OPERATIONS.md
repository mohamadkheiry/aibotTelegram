...
## پشتیبانی تغییرات بازخورد ۲۰۲۶-۰۹-۰۸

- متن غنیِ قدیمی که entity آن قبلاً ذخیره نشده فقط با ارسال دوباره متن اصلی توسط صاحب محتوا بازیابی می‌شود؛ شناسه ایموجی یا مقصد لینک را حدس نزنید.
- در فیش تازه، caption و دکمه بررسی روی خود رسانه‌اند. timeout نباید اعلان دوم بسازد؛ خطای قطعی نبود رسانه fallback متنی دارد. sent/failed قدیمی را برای تغییر ظاهر requeue نکنید.
- «غیرفعال» یادآور، pending/queued محصول را لغو می‌کند. فعال‌کردن دوباره برای تکمیل‌های آینده است؛ کارهای لغوشده خودکار زنده نمی‌شوند. متن retry تعداد روز فعلی را از تاریخ محلی حساب می‌کند، نه days_before قدیمی.
- دکمه تأیید قدیمی وضعیت تیکت پس از خروج یا پیام تازه عمداً نامعتبر است؛ کاربر از صفحه تازه اقدام کند. پشتیبان نباید status را با SQL برای دورزدن کنترل نسخه تغییر دهد.
- پیش از اطلاع‌رسانی پایان، محدودیت صوت‌ها و تصمیم کم‌واریزی در [گزارش بازخورد](CLIENT_FEEDBACK_2026-09-08.md) را بخوانید. اصلاح رابط مجوز تغییر تأیید مالی یا فعال‌کردن پل بانک/ارز نیست.

میزبان عملیاتی مصوب از ۲۰۲۶-۰۹-۰۷ فقط `192.168.10.111` است. با SSH حساب `mr-kheiry` وارد شوید و `alone-account-bot.service` را بررسی کنید؛ مسیرها و SHA در [CURRENT_DEPLOYMENT.md](CURRENT_DEPLOYMENT.md) هستند. روی Windows poller زنده اجرا نکنید و برای رفع خطا، DB قدیمی Windows را روی دادهٔ تازهٔ سرور restore نکنید. serviceهای دیگر این سرور خارج دامنهٔ عملیات این ربات‌اند.
