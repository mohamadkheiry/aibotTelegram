...
- موفقیت بدون پرداخت بیرونی: Orderهای wallet-only و تخفیف کامل که کلید `order:{id}:wallet-confirmed` یا `order:{id}:discount-confirmed` ندارند با `maintenance_zero_external_notice_cursor` صفحه‌بندی و wrap می‌شوند؛ status/ledger را برای جبران پیام تغییر ندهید.
...
برای reminder، ورودی تازه `reminder_days` فقط عدد صحیح ۱ یا بیشتر است؛ صفر legacy در schedule نادیده گرفته می‌شود. وجود `reminder:{id}` در outbox با وضعیت `queued/sending` یعنی retry در مالکیت outbox است؛ reminder را دستی `pending` نکنید. اشتراک پایان‌یافته باید reminder را بدون ارسال `cancelled` کند؛ مورد دیررسِ هنوز معتبر باید تعداد روز واقعی تا `subscription_ends_at` را نشان دهد، نه مقدار تاریخی `days_before`. در یک batch، failure دائمی گیرنده نخست باید به `failed` برسد و پردازش گیرنده‌های بعدی ادامه یابد؛ توقف کل batch نشانه regression head-of-line است.
...
- با افزودن موجودی، worker قدیمی‌ترین reservation دارای stock را در کل محصولات fulfil می‌کند؛ بیش از ۱۰۰ تحویل در یک چرخه طبیعی نیست و باقی backlog باید در چرخه بعد پیشرفت کند.
