...
ربات با `getUpdates` و یک process فعال اجرا می‌شود. SQLite schema 11 منبع حقیقت است. هویت مدیر با private chat/user ID اثبات‌شده و root marker پایدار مجوز می‌گیرد؛ update مدیریتی journal `started/completed` دارد و خطای موقت پایهٔ دیتابیس با NACK، offset ثابت و retry همان update پیش از ادامه batch بازیابی می‌شود. فهرست‌های مدیریتی users/orders/tickets و چهار تاریخچه کامل کاربر صفحه‌بندی ۲۰تایی و total دقیق دارند. Order/خلاصه نخستین خرید اتمیک ساخته می‌شود و اعلان canonical موفقیت پیش از fulfillment gate می‌شود. outbox/reconciliation آثار commit‌شده را پس از خطای شبکه یا crash بازیابی می‌کند، اما exactly-once end-to-end شبکه‌ای را تضمین نمی‌کند.
...
1. امنیت، یکپارچگی مالی و جلوگیری از افشای داده قابل نقض نیست.
2. رفتار پوشش‌داده‌شده با تست سبز و invariant مستند، قرارداد فعلی است.
3. `USE_CASES.md` و `BUSINESS.md` intent محصول را مشخص می‌کنند.
4. منابع `references/` زمینه تاریخی‌اند.
5. تعارض واقعی باید قبل از mutation به مالک محصول گزارش و نتیجه در ADR/مستندات ثبت شود.
