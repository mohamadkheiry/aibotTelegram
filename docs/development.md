...
به‌روزرسانی ۲۰۲۶-۰۹-۲۳: [چهار اصلاح طبق پاسخ کارفرما](CLIENT_DECISIONS_2026-09-23.md)، شامل نمایش معرف، اولویت پاداش محصول بر اولین خرید عمومی و تسویه دستی مبلغ واقعی کم/اضافه‌واریزی کارت. این قرارداد برای موضوع‌های مذکور جایگزین عبارت‌های «منتظر پاسخ» در گزارش‌های تاریخی پایین است. schema ۱۲ حفظ شده؛ rollback باید reader/guard تسویه مبلغ واقعی را نگه دارد. وضعیت اتصال خودکار و انتشار را از گزارش جاری بخوانید.
...
برای پاداش محصول، تصمیم ۲۰۲۶-۰۹-۱۳ در [FINANCIAL_DECISIONS_2026-09-12.md](FINANCIAL_DECISIONS_2026-09-12.md) ملاک است. guard مشترک `_validate_product_reward_exclusivity` باید در transaction ساخت و فعال‌سازی باقی بماند؛ پیش‌نمایش UI جای کنترل race را نمی‌گیرد. شروط combined با `product_ids` و مرزهای inclusive را تست کنید. قواعد عمومی را بدون پاسخ مالک حذف یا override نکنید. تست‌های اختصاصی: `python -m unittest tests.test_product_reward_exclusivity tests.test_followup_feedback -v`؛ سپس کل suite، lint و hygiene.
...
در `TelegramClient._poll_batch`، پایان retry محدود HTTP فقط برای دریافت `getUpdates` به recovery پیوسته تبدیل می‌شود. خطای transport یا پاسخ 5xx همان offset و filterها را با backoff نمایی تا سقف `max_retry_delay` دوباره می‌خواند؛ batch معتبر شمارنده را reset می‌کند. 429 با `retry_after` عددی، متناهی و نامنفی (شامل header HTTP) رعایت می‌شود و delay معتبر provider سقف نمی‌خورد. این حلقه handler، ذخیره offset یا ارسال پیام را retry نمی‌کند. 400/401/403/404/409 همچنان fail می‌شوند تا توکن نامعتبر و poller دوم پنهان نشوند. `stop_event` انتظار را قطع می‌کند و update تازه ACK نمی‌شود. log فقط کد خطا و delay محلی دارد. این رفتار در `run_polling` و `iter_updates` مشترک است؛ قرارداد ACK/NACK و اختلاف semantics دو API ثابت است. تست‌های `test_polling_recovery.py` شامل بازتولید crash واقعی 502، recovery شبکه/HTTP، flood control، shutdown، حفظ ترتیب NACK و عدم تکرار sendMessage هستند.
...
- اعلان موفقیت Order باید متن canonical شماره سفارش، محصول، مبلغ و روش را با کلید `payment:{payment_id}:order-confirmed` یا، برای کیف پول، تخفیف کامل و محصول رایگان کاربر، به‌ترتیب با کلیدهای `order:{order_id}:wallet-confirmed`، `order:{order_id}:discount-confirmed` و `order:{order_id}:free-confirmed` بسازد. همه شاخه‌های ready/reserve/manual پیش از mutation `order_success_notice_ready` را بررسی می‌کنند: نبود پیام و statusهای `queued/sending` fulfillment را به تعویق می‌اندازد؛ `sent/failed/cancelled` اجازه ادامه می‌دهد تا failure دائمی Telegram سفارش paid را strand نکند. maintenance باید missing notices را با cursor/wrap پیش از reward/fulfillment بازیابی کند.
...
- فیش فقط برای card است؛ نخستین فیش strictly پیش از expires_at پذیرفته می‌شود؛ فیش ثبت‌شده به‌موقع تا تصمیم صریح مدیر قابل بررسی و جایگزینی است و خودکار منقضی نمی‌شود. Order/topup crypto باز باید فقط از `provider_invoice_url` ذخیره‌شده و دوباره‌اعتبارسنجی‌شده با دکمه URL قابل resume باشد؛ URL خالی/legacy ناامن لینک یا callback receipt تولید نمی‌کند و مسیر پشتیبانی می‌دهد.
...
- reservation سفارش‌دار بر اساس order یکتا است؛ همه entry pointهای تخصیص ready اولویت زمان پرداخت Order، با paid_at میکروثانیه‌ای افزایشی برای داده تازه و tie-break قطعی برای داده قدیمی را در همان transaction رعایت می‌کنند. زمان ایجاد reservation یا ورود به stage جدید مجوز جلو زدن از سفارش قدیمی‌تر نیست.
...
maintenance به‌ترتیب completed evidence اعمال‌نشده را settle و سپس crypto را poll می‌کند؛ بعد alertهای review، notice تصمیم‌های provider/card، alert فیش/manual-info/no-stock/reward-event/ticket/security، انقضاهای محلیِ غیرcrypto و اعلان terminal آن‌ها را بازیابی می‌کند. سپس paid-noticeهای بیرونی و success-noticeهای wallet-only/تخفیف کامل/خرید رایگان تأییدشده کاربر missing را می‌سازد، و فقط بعد از این dependency به پاداش سفارش‌های موفق با `reward_processed_at IS NULL` و selector مستقل fulfillment همه سفارش‌های status=`paid` می‌رسد. prompt سفارش‌های `awaiting_stock`/`awaiting_info`، رزرو موجودی، fulfil سفارش ready در `processing` پس از restock، delivery سفارش ready تکمیل‌شده، reminder، outbox و batch broadcast مراحل بعدی‌اند. deadline محلی به‌تنهایی crypto را terminal نمی‌کند. marker پاداش فقط بعد از ثبت اعلان‌های پایدار زده می‌شود و هرگز selector fulfillment نیست. commit انقضا و پیام «دیگر پرداخت نکن» باید یا اتمیک باشد یا با query ردیف terminal فاقد outbox بازیابی شود؛ crash بین این دو نباید اعلان را برای همیشه گم کند.

`reminder_days` فقط integer نامنفی می‌پذیرد؛ صفر یادآوری آغاز روز محلی پایان اشتراک است. `Database(reminder_timezone=settings.timezone)` در startup استفاده می‌شود؛ ورودی منفی، اعشاری یا boolean رد می‌شود. برای صفر، اگر schedule در همان روز و پیش از پایان است موعد برابر اکنون خواهد بود؛ انقضای دقیق نیمه‌شب و اشتراک پایان‌یافته یادآوری همان روز ندارند. موعد مثبت گذشته ساخته نمی‌شود. reminderها با batch محدود claim و مستقل پردازش می‌شوند. پیش از نخستین ارسال و در `_deliver_outbound_messages` پیش از retry، پایان واقعی و مالکیت reminder دوباره کنترل می‌شود و مورد منقضی همراه outbox بدون ارسال cancelled می‌شود. متن زمان مطلق پایان و timezone را نشان می‌دهد. outbox `queued/sending` با کلید `reminder:{id}` مالک retry است؛ reminder تا stale reconciliation processing می‌ماند.
...
قراردادهای ممیزی منابع اصلی را حفظ کنید: مسیر کاربر `create_order(defer_free_confirmation=True)` و پس از تأیید دکمه پرداخت `confirm_zero_payable_order` است؛ تخفیف کامل به‌تنهایی تحویل نمی‌کند. key محصول رایگان کاربر `order:{id}:free-confirmed` و تشخیص مسیر کاربر از created-summary است. `_allocate_paid_timestamp` ترتیب پرداخت‌های تازه همان ثانیه را با میکروثانیه ثبت می‌کند؛ تمام selectorهای ready و `_assign_inventory` همان تقدم را رعایت کنند. برای tie legacy فقط شواهد reservation موجود و Order ID قابل اتکاست.

`list_expired_wallet_topups_missing_notice` و `_reconcile_expired_wallet_topup_notices` هشدار شماره/مبلغ/عدم پرداخت را با `payment:{id}:topup-expired` بازیابی می‌کنند؛ سقف صدتایی و cursor/wrap حفظ شود. فیش ثبت‌شده به‌موقع تا تصمیم صریح مدیر باز می‌ماند. متن محصول/FAQ و اطلاعات manual/ticket را کامل و HTML-safe تقسیم کنید؛ clamp جایگزین تمام محتوا نیست. نوع فارسی تراکنش مستقل از reason نمایش داده شود. فهرست‌های مدیریتی admin/category/product/inventory/discount/reward/FAQ صفحات ۲۰تایی دارند؛ syntax نهایی در `ADMIN_GUIDE_FA.md` است.

تست‌های پذیرش این قراردادها در `test_spec_customer_audit.py`، `test_spec_admin_audit.py`، `test_spec_purchase_audit.py` و `test_spec_reminders_transactions.py` هستند؛ این تست‌ها جایگزین منابع اصلی نیازمندی نیستند.
...
ابتدا [BUTTON_UI.md](BUTTON_UI.md) را بخوانید. `admin_forms.py` کاتالوگ ۸۳ عملیات و فیلدهای شرطی است؛ `admin_ui.py` فرم پایدار با state `admin:ui` را اجرا می‌کند و `admin.py` همان handler و قواعد دامنه مشترک را نگه می‌دارد. برای نیازمندی تازه ابتدا دکمه/فرم، نقش و تست UI تعریف کنید؛ افزودن یک فرمان تنها، کافی نیست. رابط عمومی Telegram فقط start را معرفی می‌کند و فرمان‌های قبلی حذف نشده‌اند.

`_command_parts` در context فرم مقادیر جداگانه را lossless تحویل handler می‌دهد؛ `|` داخل پیام/عنوان delimiter نیست. `_send` پاسخ‌های مدیریتی را تا ثبت وضعیت done در حافظه buffer می‌کند؛ اعلان حیاتی همچنان outbox دامنه را دارد. این buffer صندوق پیام پایدار جدید نیست: در crash ممکن است مدیر مجبور به مشاهده نتیجه از فهرست شود. در حالت read list، `_send_page` تمام ردیف‌ها را می‌فرستد و navigation دکمه‌ای page/filter را حفظ می‌کند. UI mutation SQL ندارد و schema همان ۱۱ است.

تست‌های جدید در `tests/test_admin_buttons.py`: پوشش همه عملیات، role و revoke، کاربر عادی/مدیر، انتخاب/جست‌وجو/صفحه، داده حاوی pipe، انبار بدون echo، مالی و toggle replay، crash بعد از آخرین ورودی و complete journal، فیش و تیکت، تخفیف و پاداش ترکیبی، پیام گروهی و لغو کاربر. تست دامنه جایگزین تست پیمایش فرم نیست و بالعکس.

### افزودن handler سازگار
