...
پنل دکمه‌ای مجوز جدیدی نمی‌سازد. `admin_ui.authorise` role فعال/verifyشده را با admin ID و private chat ID از DB دوباره می‌خواند؛ token/revision فرم و actor نیز بررسی می‌شوند. ورودی selector فقط جست‌وجو است و انتخاب نهایی از options ذخیره‌شده می‌آید؛ handler همان validation دامنه را دارد. متن pipe به آرایه اصلی منتقل و secret در preview مخفی می‌شود. دکمه stale و تأیید دوم اجرا نمی‌شوند؛ NACK و replay همان journal قبلی حفظ شده‌اند. اعلان‌های تاریخی مستقیم رفتار سازگار خود را دارند. [BUTTON_UI.md](BUTTON_UI.md) مرزهای دقیق و تست‌ها را شرح می‌دهد.

## دامنه

خروج متنی از فرم نیز مشمول guard خاموشی/جوین است؛ پاک‌کردن state نباید راه میان‌بُر نمایش منوی عمومی شود. دریافت اطلاعات سفارش علاوه بر مالکیت و status، snapshot نوع manual را در render، callback و state بررسی می‌کند تا readyِ در انتظار موجودی اطلاعات غیرضروری جمع نکند. سه لایهٔ رابط جای کنترل تراکنشی نهایی را نمی‌گیرند. regressionهای مربوط در [ممیزی سرتاسری](FULL_SPEC_AUDIT.md) ثبت شده‌اند.
...
- `python -m app.main --check` نیز روی همان دیتابیس schema را تا نسخه ۱۲ migrate و owner bootstrap را ایجاد/اعتبارسنجی می‌کند؛ conflict identity پیش از `getMe` fail می‌شود. این preflight را فقط بعد از backup و با identity نهایی release اجرا کنید.
...
- خطای blocked recipient نباید update queue یا batch reminder را متوقف کند؛ پس از ساخته‌شدن outbox، همان outbox مالک retry است. در ارسال نخست و هر retry، پایان واقعی و مالکیت reminder دوباره کنترل می‌شود؛ reminder پایان‌یافته همراه outbox بدون ارسال cancelled می‌شود. متن روزهای مثبت زمان مطلق پایان دارد تا در قطع شبکه و عبور از روز بعد ادعای نسبی قدیمی نکند؛ روز صفر «امروز» همراه ساعت پایان است.
...
- فیش فقط card است؛ نخستین فیش باید پیش از `expires_at` برسد. فیش ثبت‌شده به‌موقع در verifying تا تصمیم صریح مدیر قابل بررسی و جایگزینی است؛ گذشت زمان به‌تنهایی آن را منقضی نمی‌کند. مالکیت، نوع پرداخت و وضعیت غیرterminal در هر ارسال دوباره کنترل می‌شوند. crypto باز فقط با `provider_invoice_url` ذخیره‌شده‌ای که در render نیز safe-HTTPS validation را پاس کند قابل resume است؛ URL خالی/legacy ناامن نه لینک می‌سازد و نه به receipt callback تبدیل می‌شود.
...
- قبل از approval، cardبودن intent، مالکیت/مجوز، وجود فیش پذیرفته‌شده به‌موقع و وضعیت باز دوباره بررسی شود؛ تأخیر مدیر به‌تنهایی فیش معتبر را منقضی نمی‌کند و وضعیت terminal قدیمی احیا نمی‌شود.
...
- paid payment فاقد اعلان و Order موفق wallet-only/تخفیف کامل/خرید رایگان تأییدشده کاربر فاقد success notice باید توسط maintenance با query/cursor پایدار به outbox برگردد؛ recovery اعلان نباید settlement یا wallet capture/credit را تکرار کند. fulfillment تا status اعلان canonical از `queued/sending` خارج نشده است بسته می‌ماند؛ `sent` یا failure/cancellation terminal اجازه ادامه می‌دهد تا failure دائمی Telegram Order پرداخت‌شده را strand نکند. failure terminal باید در outbox/alert عملیاتی دیده شود و با SQL به retry نامحدود برنگردد.
