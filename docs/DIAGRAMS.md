...
flowchart LR
    ...
    subgraph Service["ارتباط و نگهداری"]
        ...
        Ticket["ایجاد/ادامه تیکت و بستن/بازگشایی تأییدشده تیکت خود"]
...
flowchart TD
    ...
    L["append پیام با message ID یکتا؛ حفظ همه متن‌ها و فایل‌ها"]
    L0{"پایان ارسال اطلاعات زده شد؟"}
    L1["collecting باز؛ ادامه یا بازگشت بدون حذف اطلاعات"]
    M["finish اتمیک: processing و outbox تأیید؛ پاک‌کردن state همان مجموعه"]
...
flowchart TD
    ...
    K -->|"فیش دستی"| N --> O --> P
    P -->|"مبلغ دقیق"| W
    P -->|"مبلغ متفاوت؛ بررسی واقعی بانک و شماره پیگیری"| PA{"دریافت واقعی برای سفارش کافی است؟"}
    PA -->|"کم‌واریزی"| PB["لغو سفارش؛ آزادسازی hold و تخفیف؛ کل دریافت به کیف پول و notice اتمیک"] --> AB
    PA -->|"کافی / اضافه"| PC["تسویه بدهی سفارش؛ مازاد به کیف پول؛ notice اتمیک"] --> AAG
    PA -->|"شارژ کیف پول"| PD["اعتبار دقیق مبلغ واقعی و notice اتمیک"] --> AB
    P -->|"خیر"| Q --> AB
...
flowchart TD
    ...
    E["یک پیام شرح یا caption و پیوست اختیاری"]
    ...
    N -->|"پاسخ کاربر"| K --> H
    N -->|"پاسخ مدیر"| L --> M --> I
    N -->|"بستن توسط مدیر"| O --> Z
    N -->|"بستن توسط مشتری"| Confirm
    J -->|"بله؛ درخواست بازگشایی مشتری"| Confirm
    Confirm["prompt تأیید با nonce و snapshot مکالمه"] --> Choice{"تأیید صریح همان prompt؟"}
...
flowchart TD
    ...
    I["ارزیابی با زمان خرید؛ پاداش صریح محصول بر first_purchase عمومی مقدم؛ درصد از subtotal قبل تخفیف"]
...
flowchart TB
    ...
    Product --> Fields["اطلاعات و ویرایش<br/>مشخصه‌های مرتبط با فرمت محصول"]
    ...
    Format --> Manual["نیازمند اطلاعات<br/>متن درخواست و تکمیل؛ بدون انبار آماده"]
    Stock --> Limit["ready: سقف آیتم تحویل‌نشده<br/>شامل آماده و غیرفعال"]
    Product --> Rewards["پاداش معرف همین محصول<br/>قواعد ثابت و وضعیت؛ بدون تغییر قواعد عمومی"]
    Rewards --> Form
    Limit --> Form
...
flowchart TB
    ...
    C --> D["خواندن تنظیم و نسخه بخش/والد؛ افزودن فقط slot تازه به نسخه قدیمی"]
