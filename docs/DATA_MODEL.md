...
پیگیری مالی ۲۰۲۶-۰۹-۱۲ schema را به نسخه ۱۲ می‌برد: `reward_rules.amount_mode` نوع ثابت/درصد و `maximum_amount` سقف اختیاری را نگه می‌دارند؛ ردیف‌های قدیمی با `fixed` backfill می‌شوند. selector پاداش سفارش فقط `completed` را می‌خواند. اصلاحات تیکت ۲۰۲۶-۰۹-۰۹ و قرارداد replay آن‌ها بدون تغییر حفظ شده‌اند. [شاهد و محدوده](PENDING_FEEDBACK_2026-09-09.md).
...
قابلیت چیدمان نخست در schema 11 و بدون جدول تازه افزوده شد و در schema 12 نیز در `settings` با کلید `customer_layout:SECTION` ذخیره می‌شود: سند current/version/history (حداکثر ۱۰ نسخه)/updated_by/updated_at؛ `user_states` با state `admin:layouts`، draft و actor/chat/token/revision/phase/نسخهٔ مبنا. ذخیره و effect `customer-layout` در همان `processed_admin_updates` اتمیک‌اند. config هیچ اطلاعات شخصی سفارش/تیکت یا action payload ندارد. `customer_layouts_enabled` کلید عملیاتی پیش‌فرض true برای خاموش‌کردن صرفاً reflow است. [طرح و invariant دقیق](CUSTOMER_LAYOUTS.md).
...
رابط دکمه‌ای نخست بدون migration در schema 11 افزوده شد و قرارداد آن در schema 12 ثابت است: `user_states` از مقدار `admin:ui` برای action/actor/chat، token، revision، step، مقادیر مستقل، گزینه‌ها، last_input و وضعیت editing/confirm/executing/done استفاده می‌کند. executing هویت تأیید را پیش از handler نگه می‌دارد؛ message ID داخلی از token فرم ساخته می‌شود. secret پس از موفقیت از state حذف می‌شود. جدول processed_admin_updates و effect یکتای handler تغییر نکرده است. ساختار و crash boundary در [BUTTON_UI.md](BUTTON_UI.md).

## خلاصه

جوین اجباری از state مرور `admin:joins` با kind=list/channel، id، page، actor/chat و prompt_message_id استفاده می‌کند؛ فرم آن `return_to.scope=joins` دارد. این قابلیت migration مستقلی ندارد و schema 12 نیز کانال‌ها را در rollout بازنویسی نمی‌کند. [جزئیات](ADMIN_JOINS.md).
...
`amount_mode=fixed` مقدار `amount` را تومان می‌داند. در `amount_mode=percent`، `amount` درصد صحیح ۱ تا ۱۰۰ است؛ مبلغ نهایی `floor(subtotal_amount * amount / 100)` و سپس حداقل آن با `maximum_amount` اختیاری است. مبنا قیمت snapshotشده پیش از تخفیف و مستقل از روش پرداخت است. درصد برای `start` مجاز نیست.
