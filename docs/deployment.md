...
اصلاح UI-A05/UI-A06 فقط زمینهٔ فرم روش پرداخت را می‌افزاید؛ بدون migration و بدون تغییر flagهای روش‌هاست. پس از backup و restart تک‌نمونه‌ای، روش را از فرم انتخاب، نام و وضعیت فعلی را مشاهده و لغو کنید. آزمون تغییر فعال/غیرفعال فقط روی DB موقت انجام شود. فرم‌های باز قبلی و callbackهای موجود معتبرند؛ rollback فقط نمایش این زمینه را حذف می‌کند.

اصلاح UI-A03/UI-A04 دکمه‌های ربات/تعمیرات بدون migration است و وضعیت ذخیره‌شدهٔ `bot_enabled` در rollout حفظ می‌شود. پس از backup و restart تک‌نمونه‌ای، `/start` و «مدیریت کلی ربات» دکمه‌های متناسب با وضعیت فعلی را نشان می‌دهند؛ فرم را باز و لغو کنید. آزمون رفت‌وبرگشت فعال/غیرفعال روی DB موقت انجام شود تا دسترسی مشتریان واقعی مختل نشود. rollback، label ثابت قبلی را برمی‌گرداند ولی state/DB نیاز به پاک‌سازی ندارد.

نسخه جوین اجباری هم بدون migration است: با `/start` به «مدیریت کلی ربات ← جوین اجباری» بروید؛ فهرست وضعیت‌ها، سه دکمه کانال، لغو فرم و بازگشت را بدون تغییر کانال واقعی smoke کنید. هنگام rollback به نسخه فاقد `admin:joins`، `/start` بزنید؛ offset یا state را با SQL پاک نکنید. [شرح و محدودیت تست](ADMIN_JOINS.md).

نسخهٔ ناوبری ۹بخشی نیز migration ندارد. پس از backup، توقف نمونه قبلی و start تک‌نمونه‌ای، مالک از «پنل مدیریت ← محصولات ← دسته ← محصول» وارد اطلاعات، انبار و فرمت شود و بدون ثبت داده آزمایشی در production، لغو/بازگشت را بررسی کند. پیام‌های root تاریخی با ۱۳ گروه خودکار بازنویسی نمی‌شوند؛ `/start` پنل جدید را می‌دهد. callbackهای قبلی برای سازگاری مانده‌اند. هنگام rollback به نسخه فاقد `admin:catalog`، مدیر `/start` بزند؛ state یا offset را با SQL پاک نکنید. هیچ انتقال دسته، تغییر فرمت، موجودی، سفارش یا دسترسی مدیران بخشی از rollout نیست. [جزئیات پذیرش](ADMIN_HIERARCHY.md).

اصلاح UI-A01/UI-A02 نیز بدون migration است. state پیشین بدون `prompt_message_id` پشتیبانی می‌شود؛ تغییر UI نباید offset یا state کاربران را با SQL پاک/جلو ببرد. بعد از rollout، FAQ خالی → «همه / بدون محدودیت» و لمس دکمهٔ کش‌شده را smoke کنید: نتیجه/دکمهٔ refresh باید قابل دسترسی باشد، بدون اجرای دوبارهٔ mutation. جزئیات و مرز تست‌های بدون Telegram واقعی در [BUTTON_UI_AUDIT.md](BUTTON_UI_AUDIT.md).
...
Smoke test بدون تراکنش واقعی: ورود مالک، مشاهده ۹ بخش، جست‌وجوی کاربر، بازگشت/لغو فرم، دسته و محصولات همان دسته و نبود پنل برای مشتری عادی. جزئیات در [BUTTON_UI.md](BUTTON_UI.md). rollback کد قبلی schema را تغییر نمی‌دهد؛ اگر نسخه مقصد state جدید را نمی‌شناسد، کاربر با `/start` آن را لغو و دوباره شروع کند. توکن یا DB را برای این تغییر در گیت قرار ندهید.
