...
کاربر `192.168.10.111` را برای همهٔ انتشارهای بعدی تعیین کرد. دسترسی SSH و Ubuntu، Python 3.14.4، همگامی ساعت و HTTPS خروجی Telegram تأیید شدند. واحد اختصاصی مقصد `alone-account-bot.service` و مسیرهای آن مطابق [deployment.md](deployment.md) هستند. رمز دسترسی خارج مخزن می‌ماند. در زمان آماده‌سازی این commit، cutover هنوز انجام نشده و شرح Windows در بخش‌های بعدی وضعیت مبدأ/تاریخچه است؛ موفقیت انتقال فقط پس از backup، توقف مبدأ، تطبیق داده و verification مقصد ثبت خواهد شد.
