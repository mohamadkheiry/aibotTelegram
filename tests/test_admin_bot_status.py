"""State-aware bot/maintenance controls, exercised from emitted keyboards."""
...
class AdminBotStatusTests(unittest.TestCase):
    ...
    def assert_controls(self, enabled):
        ...
        self.assertEqual([b["text"] for b in controls],
                         ["غیرفعال‌کردن ربات", "فعال‌کردن حالت تعمیرات"] if enabled else
    ...
    def test_active_settings_offer_disable_bot_and_enable_maintenance(self):
        self.open_settings()
        self.assert_controls(True)
        self.assertTrue(any("حالت تعمیرات: غیرفعال" in m["text"] for m in self.telegram.messages))

    def test_inactive_settings_offer_enable_bot_and_disable_maintenance(self):
        self.db.set_setting("bot_enabled", False)
        self.open_settings()
        self.assert_controls(False)
        self.assertTrue(any("حالت تعمیرات: فعال" in m["text"] for m in self.telegram.messages))
    ...
    def test_maintenance_control_round_trip_stays_in_sync_with_bot_access(self):
        self.open_settings()
        self.click(label="فعال‌کردن حالت تعمیرات")
        self.assertEqual(self.state()["action"], "bot_off")
        self.click(label="تأیید و اجرا")
        self.assert_controls(False)
        self.click(label="غیرفعال‌کردن حالت تعمیرات")
        self.assertEqual(self.state()["action"], "bot_on")
        self.click(label="تأیید و اجرا")
        self.assertTrue(self.db.get_setting("bot_enabled"))
        self.assert_controls(True)

    def test_confirmation_explains_current_target_and_cancel_does_not_change_state(self):
        for enabled, label in ((True, "فعال‌کردن حالت تعمیرات"), (False, "غیرفعال‌کردن حالت تعمیرات")):
            with self.subTest(enabled=enabled):
                ...
                self.assertIn("ربات غیرفعال و حالت تعمیرات فعال" if enabled else
                              "ربات فعال و حالت تعمیرات غیرفعال", original["text"])
