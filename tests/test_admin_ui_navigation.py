...
class AdminNavigationTests(unittest.TestCase):
    ...
    def test_noncatalog_actions_are_reachable_through_nested_emitted_buttons(self):
        # Entity-centred views and state-aware bot controls have dedicated
        # traversal tests. All 83 handlers retain the
        # independent routing test in test_admin_buttons.
        visited = set()
        specialized = {"joins", "join_add", "join_toggle", "join_delete", "bot_on", "bot_off"}
        for group, title in GROUPS.items():
            ...
            for action in (a for a in ACTIONS.values() if a.group == group and a.key not in specialized):
        ...
        self.assertEqual(visited, {a.key for a in ACTIONS.values() if a.group not in {"catalog", "inventory"} and a.key not in specialized})
