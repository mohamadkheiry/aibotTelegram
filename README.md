...
ادامه اصلاحات و موارد باز: [گزارش پیگیری](docs/FOLLOWUP_FEEDBACK_2026-09-08.md). شماره سفارش تازه افزایشی و اطلاعات manual چندپیامی با پایان صریح است؛ تنظیم قواعد ثابت معرف از صفحه محصول نیز در دسترس است. درصد/سقف و زمان پرداخت پاداش هنوز نیازمند تصمیم مالک‌اند.

ظاهر Premium اختیاری: ۴۵ آیکون خطی Lucide با custom emoji ID برای دکمه‌های inline؛ رنگ‌های سند با `BUTTON_COLOR_MODE=colored` حفظ می‌شوند و متن و عملکرد دکمه‌ها تغییر نمی‌کند. `theme` فقط fallback انتخابی با تأیید مالک است. [راهنمای آیکون‌ها، انتشار بسته و انتقال ربات](docs/BUTTON_ICONS.md).

مدیر می‌تواند چیدمان همهٔ ۳۹ نوع صفحهٔ کاربران را از «مدیریت کلی ربات ← چیدمان دکمه‌های کاربران» با پیش‌نمایش، جابه‌جایی، یک تا سه ستون و انتشار تأییدشده تغییر دهد. ترتیب تک‌تک گزینه‌های کاتالوگ، تنظیم موردی، بازگشت به چیدمان قبلی و بازنشانی اولیه فراهم است؛ چیدمان سند پیش‌فرض باقی می‌ماند. [راهنمای کامل](docs/CUSTOMER_LAYOUTS.md).

جوین اجباری: «مدیریت کلی ربات ← جوین اجباری ← کانال». هر کانال نشان وضعیت ✅/❌ و صفحهٔ تغییر وضعیت، حذف و بازگشت دارد. این تنها استثنای درخواست‌شده برای emoji در دکمه‌هاست. [راهنما و تست‌ها](docs/ADMIN_JOINS.md).

پنل مدیریت طبق سند ۹ بخش اصلی دارد. مسیر مدیریت کالا: «محصولات ← دسته/زیردسته ← محصول ← اطلاعات و ویرایش، انبار، فرمت». همه ۲۳ مشخصه از داخل همان محصول قابل مشاهده و ویرایش‌اند؛ افزایش موجودی، تخصیص اکانت و انتخاب فرمت نیز در همان مسیرند. [نقشه و تست‌های این ساختار](docs/ADMIN_HIERARCHY.md).

رابط اصلی دکمه‌محور است: پس از `/start` مشتری منوی اصلی سند و مدیر تأییدشده دکمه «پنل مدیریت» را می‌بیند. همه ۸۳ عملیات مدیریتی فرم/انتخابگر دکمه‌ای دارند؛ فقط داده‌هایی مانند نام، مبلغ و متن تایپ می‌شوند. راهنما و قرارداد توسعه: [BUTTON_UI.md](docs/BUTTON_UI.md). فرمان‌های قبلی صرفاً مسیر اختیاری سازگاری‌اند.
...
4. برنامه در شروع فقط فرمان شروع را با `setMyCommands` معرفی می‌کند و webhook قدیمی را حذف می‌کند؛ بقیه قابلیت‌ها از دکمه‌ها در دسترس‌اند.
...
در تلگرام `/start` را بفرستید و برای مدیریت دکمه «پنل مدیریت» را انتخاب کنید. راهنمای کار روزانه [دکمه‌ها و فرم‌ها](docs/BUTTON_UI.md) است.

## مرجع اختیاری فرمان‌های سازگار
...
حالت پیش‌فرض `BUTTON_COLOR_MODE=colored` رنگ‌های مرجع فعلی را حفظ می‌کند. اگر در یک کلاینت مانند Telegram Web نوشته و پس‌زمینه هم‌رنگ دیده شدند، fallback اختیاری `BUTTON_COLOR_MODE=theme` را فعال کنید تا رنگ اجباری ارسال نشود و کلاینت جفت رنگ را تعیین کند. رنگ نوشتهٔ دکمه API مستقل ندارد. ترتیب، label، آیکون اختیاری و عملکرد دکمه‌ها تغییر نمی‌کنند.

پس از تغییر این مقدار در env، فقط یک process را restart و `/start` یا `/menu` ارسال کنید؛ پیام‌های قبلی خودکار بازنویسی نمی‌شوند. نگاشت style در حالت پیش‌فرض `colored`:
